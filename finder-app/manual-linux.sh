...
mkdir -p rootfs
cd rootfs
mkdir -p bin dev etc home lib lib64 proc sbib sys tmp usr var
...
PROGRAM_INTERPRETER_NAME="$(${CROSS_COMPILE}readelf -a bin/busybox | grep "program interpreter" | sed -n 's/.*: *\(.*\)]/\1/p')"
cp "${TOOLCHAIN_SYSROOT}${PROGRAM_INTERPRETER_NAME}" "${OUTDIR}/rootfs/lib"
